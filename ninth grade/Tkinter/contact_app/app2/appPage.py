...
class AppPage(tk.Frame):
	...
	def create_right_content(self):
		...
		self.update_content.grid(row=0, column=0, sticky='nesw')
		...
		for row in range(rows):
			aRow = []
			column = 1
			label = tk.Entry(self.update_content, font=("Arial", 12), bg="white")
			aRow.append(label)
			if column == 0:
				sticky = "e"
			else:
	...
	def create_right_footer(self):
		...
		update_features = ['Cancel', 'Save']
		self.update_button_features = []
		...
		for update_feature in update_features:
			button = tk.Button(self.update_footer, text=update_feature, bg="white", fg="black", font=("Arial", 12, "bold"), bd=0)
			button.grid(row=0, column=update_features.index(update_feature), sticky="nsew", padx=20, pady=(0, 10))
			self.update_button_features.append(button)
	...
	def clicked_update_btn(self):
		...
		for row in range(rows):
			...
			for column in range(columns):
				...
				entry = tk.Entry(self.update_content, font=("Arial", 12), bg="white")
